...
through_bore_encoder_t
    encoder2; // If you have a second encoder, configure it similarly
...
void full_motor_init() {
  ...
  motor_t frontLeft = {.pwm_pin = 17,
                       .group_id = 0,
                       .timer_id = 0,
                       .oper_id = 0,
                       .type = DC_MOTOR};
  motor_t frontRight = {.pwm_pin = 21,
                        .group_id = 0,
                        .timer_id = 0,
                        .oper_id = 0,
                        .type = DC_MOTOR};
  motor_t backLeft = {.pwm_pin = 25,
                      .group_id = 0,
                      .timer_id = 1,
                      .oper_id = 1,
                      .type = DC_MOTOR};
  motor_t backRight = {.pwm_pin = 32,
                       .group_id = 0,
                       .timer_id = 1,
                       .oper_id = 1,
                       .type = DC_MOTOR};
  motor_t eliServo1 = {.pwm_pin = 33,
                       .group_id = 0,
                       .timer_id = 2,
                       .oper_id = 2,
                       .type = SERVO_MOTOR};
  motor_t eliServo2 = {.pwm_pin = 4,
                       .group_id = 0,
                       .timer_id = 2,
                       .oper_id = 2,
                       .type = SERVO_MOTOR};
  motor_t eliServo3 = {.pwm_pin = 27,
                       .group_id = 0,
                       .timer_id = 2,
                       .oper_id = 2,
                       .type = SERVO_MOTOR};
  motor_t lazyTong = {.pwm_pin = 3,
                      .group_id = 1,
                      .timer_id = 0,
                      .oper_id = 0,
                      .type = SERVO_MOTOR};
  ...
  // outtake_reset logic is skipped since we lack the GPIO configuration for it
  // here for now.
}
...
typedef enum { IDLE, DUCKS, ANTENNA1, ANTENNA2, ANTENNA4, END } state_t;

void antenna1_action() {

  move_distance_encoder(robot_singleton.omniMotors, BACKWARD, 30, 25,
                        &encoder2);
  ...
  vTaskDelay(pdMS_TO_TICKS(2000));
  move_distance_encoder(robot_singleton.omniMotors, FORWARD, 30, 661,
                        &encoder2);
  vTaskDelay(pdMS_TO_TICKS(2000));
...
void antenna2_action() {
  ...
  vTaskDelay(pdMS_TO_TICKS(2000));
  move_distance_encoder(robot_singleton.omniMotors, FORWARD, 30, 100,
                        &encoder2);
  vTaskDelay(pdMS_TO_TICKS(2000));
  move_distance_encoder(robot_singleton.omniMotors, LEFT, 30, 85, &encoder1);
  vTaskDelay(pdMS_TO_TICKS(2000));
  move_distance_encoder(robot_singleton.omniMotors, FORWARD, 30, 662,
                        &encoder2);
  vTaskDelay(pdMS_TO_TICKS(2000));
  ...
  servo_set_angle(&robot_singleton.eliServo1, 900);
  vTaskDelay(pdMS_TO_TICKS(2000));

  servo_set_angle(&robot_singleton.eliServo2, 0);
  vTaskDelay(pdMS_TO_TICKS(2000));

  servo_set_angle(&robot_singleton.eliServo1, 200);
...
void app_main(void) {
  ...
  bool state_executed1 =
      false; // For states with multiple steps like ANTENNA1/2/4
  bool state_executed2 =
      false; // For states with multiple steps like ANTENNA1/2/4
  bool state_executed3 =
      false; // For states with multiple steps like ANTENNA1/2/4
  ...
  while (currentState != END) {
    ...
    case ANTENNA1:
      if (!state_executed2) {
        ...
        vTaskDelay(
            pdMS_TO_TICKS(4000)); // Simulate processing delay before action
        // run_antenna_path();
        antenna1_action();
